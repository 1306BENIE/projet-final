...
class RecommendationService {
        ...
        try {
            ...
            for (const review of history.reviews) {
                ...
                if (tool && review.rating >= 4) {
                    const categoryCount = preferences.categories.get(tool.category.toString()) || 0;
                    preferences.categories.set(tool.category.toString(), categoryCount + 2); // Donner plus de poids aux avis positifs
                }
            }
            return preferences;
        }
    ...
    async generateRecommendations(userId, preferences, limit) {
        try {
            ...
            const query = {
                owner: { $ne: new mongoose_1.Types.ObjectId(userId) }, // Exclure les outils de l'utilisateur
                isAvailable: true,
            };
