...
export const createBooking = async (req: Request, res: Response, next) => {
  try {
    ...
    logger.info(
      "[Booking] Booking créé (objet complet):",
      JSON.stringify(booking)
    );
    logger.info("[Booking] Booking créé avec _id (direct):", booking._id);
...
export const getBookingById = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      "[getBookingById] Booking found and user authorized, sending booking."
    );
    // Mapping pour renvoyer un objet Booking "plat" conforme à l'interface frontend
    const bookingResponse = {
      id: booking._id.toString(),
      userId:
        typeof booking.renter === "object" && booking.renter
          ? booking.renter._id.toString()
          : booking.renter.toString(),
      toolId:
        typeof booking.tool === "object" && booking.tool
...
export const getBookedDates = async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error("Erreur lors de la récupération des dates réservées:", error);
    res.status(500).json({
